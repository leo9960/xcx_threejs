...
var app = getApp();
var THREE = ((typeof app == "object")? app.THREE : require("three.js"));
var PANOLENS = { REVISION: '9' };
PANOLENS.init = function (canvas, wxEnv){
  PANOLENS.canvas = canvas;
  PANOLENS.wxEnv = wxEnv;
  PANOLENS.currentorientation = 0;
  PANOLENS.FPS = 30;
  PANOLENS.FPSTimeRange = 1000 / PANOLENS.FPS;
  PANOLENS.SetFPS = function (val) {
    PANOLENS.FPS = Math.min(Math.max(val, 12), 60);
    PANOLENS.FPSTimeRange = 1000 / PANOLENS.FPS;
    return PANOLENS.FPS;
  }
  /*PANOLENS.requestAnimationFrame = function (callback) {
    PANOLENS.NowTimestamp = new Date().getTime();
    var SetTime = PANOLENS.NowTimestamp - PANOLENS.LastTimestamp;
    if (SetTime < PANOLENS.FPSTimeRange) {
      var DelayTime = PANOLENS.FPSTimeRange - SetTime;
      setTimeout(PANOLENS.requestAnimationFrame, DelayTime);
      return true;
    }
    callback();
    PANOLENS.LastTimestamp = new Date().getTime();
  };*/
  PANOLENS.requestAnimationFrame = canvas.requestAnimationFrame
  PANOLENS.cancelAnimationFrame = canvas.cancelAnimationFrame;
  return PANOLENS;
}
;/**
...
THREE.OrbitControls = function ( object, domElement ) {
    ...
    var element = PANOLENS.canvas;
        ...
        if (state !== STATE.TOUCH_DOLLY) return;
				...
				var dy = event.touches[ 0 ].pageY - event.touches[ 1 ].pageY;
				var distance = Math.sqrt( dx * dx + dy * dy );
        event.scale = this.lastdistance > distance?0.5:1.5;
...
THREE.DeviceOrientationControls = function( camera, domElement ) {
	...
	var onScreenOrientationChangeEvent = function(event) {

    scope.screenOrientation = event;
	...
	this.connect = function() {
		...
		scope.domElement.addEventListener( 'deviceorientation', this.update.bind( this ), false );
...
GSVPANO.PanoLoader = function (parameters) {
	...
	PANOLENS.Utils.ImageLoader.load = function ( canvas,url, onLoad, onProgress, onError ) {
	 ...
	 */
	PANOLENS.Utils.TextureLoader.load = function ( canvas,url, onLoad, onProgress, onError ) {

		var texture = new THREE.Texture(); 

		PANOLENS.Utils.ImageLoader.load( canvas,url, function ( image ) {
...
PANOLENS.StereographicShader = {
	 ...
	 */
	PANOLENS.ImagePanorama = function ( canvas,image, radius ) {

    this.canvas = canvas;
    this.container = canvas;
		radius = radius || 500;
		var geometry = new THREE.SphereGeometry( radius, 25, 8 ),
	...
	PANOLENS.ImagePanorama.prototype.load = function ( src ) {
      ...
      //this.onLoad(new THREE.TextureLoader().load(this.canvas, src));
      new THREE.TextureLoader().load(this.canvas, src, this.onLoad.bind(this), this.onProgress.bind(this), this.onError.bind(this))
			//PANOLENS.Utils.TextureLoader.load( this.canvas,src, this.onLoad.bind( this ), this.onProgress.bind( this ), this.onError.bind( this ) );
    ...
    PANOLENS.requestAnimationFrame(function(){
	 ...
	 */
  PANOLENS.LittlePlanet = function ( canvas,type, source, size, ratio ) {
		type = type || 'image';
    this.container = canvas;
    type === 'image' && PANOLENS.ImagePanorama.call(this, canvas, source, size );
    ...
    this.frameId = PANOLENS.requestAnimationFrame( this.onUpdateCallback.bind( this ) );
      ...
      PANOLENS.cancelAnimationFrame( this.frameId );
	 ...
	 * @param {number} [ratio=0.5]  - Ratio of plane geometry's height against width
	 */
	PANOLENS.ImageLittlePlanet = function ( canvas,source, size, ratio ) {
    this.canvas = canvas;
    this.container = canvas;
    PANOLENS.LittlePlanet.call(this, canvas,'image', source, size, ratio );
  ...
  PANOLENS.ImageLittlePlanet.prototype.onLoad = function (texture ) {
	 ...
	 */
	PANOLENS.Reticle.prototype.loadTextures = function (canvas) {
    ...
    this.idleTexture = new THREE.TextureLoader().load(canvas, this.idleImageUrl);
    this.dwellTexture = new THREE.TextureLoader().load(canvas, this.dwellImageUrl);
	...
	PANOLENS.Reticle.prototype.select = function ( completeCallback ) {
      ...
      this.timerId = PANOLENS.requestAnimationFrame( this.select.bind( this, completeCallback ) );
	...
	PANOLENS.Widget.prototype.createMainMenu = function ( menus ) {
      ...
      PANOLENS.requestAnimationFrame( onNextTick );
	 ...
	 * @param {array}   [options.controlButtons=[]] - Button names to mount on controlBar if controlBar exists, Defaults to ['fullscreen', 'setting']
	...
	PANOLENS.Viewer = function ( options ) {
		...
		options.controlButtons = options.controlButtons || [ 'fullscreen', 'setting' ];
      ...
      console.warn("cannot create canvas.");
      return false;

		}

		this.container = container;
    this.requestAnimationFrame = PANOLENS.requestAnimationFrame;
    this.cancelAnimationFrame = PANOLENS.cancelAnimationFrame;
    this.wxEnv = options.wxEnv;
		this.camera = options.camera || new THREE.PerspectiveCamera( this.options.cameraFov, this.container.clientWidth / this.container.clientHeight, 1, 10000 );
		this.scene = options.scene || new THREE.Scene();
    this.scene.background = new THREE.Color(0xff0000);
    this.renderer = options.renderer || new THREE.WebGLRenderer({ canvas:container,alpha: true, antialias: false } );
		...
		// Renderer
    //this.renderer.setPixelRatio( this.container.pixelRatio );
    ...
    if (!this.reticle.textureLoaded) { this.reticle.loadTextures(this.container); }
	 ...
	 */
  PANOLENS.Viewer.prototype.onWindowResize = function (event) {
    var windowWidth = event.size.windowWidth;
    var windowHeight = event.size.windowHeight;
  ...
  PANOLENS.Viewer.prototype.render = function () {
    ...
    if (this.timestamp > 100) {
      var cam = { position: this.camera.position, rotation: this.camera.rotation};
      this.wxEnv.setData({ camera: cam });
      this.timestamp = 0;
    }*/
		this.requestAnimationId = this.requestAnimationFrame( this.animate.bind( this ) );
		this.onChange();
	...
	PANOLENS.Viewer.prototype.registerEventListeners = function () {
		...
		// Resize Event
		this.container.addEventListener( 'resize' , this.HANDLER_WINDOW_RESIZE, true );
